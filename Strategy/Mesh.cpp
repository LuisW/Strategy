...
Mesh* LoadTEMMesh_0_9_2(std::ifstream& file)
{
	bool fmt[6];
	file.read((char*)fmt, sizeof(bool)* 6);
	...
	if (fmt[1])				//Tangents
	{
		format.addElem(vertSize, GL_FLOAT, 3, SEM_TANGENT, 0);
		vertSize += 3 * sizeof(float);
	}
...
Mesh* MeshAssetManager::Load(MeshKey key)
{
	...
	case 920:
		return LoadTEMMesh_0_9_2(file);
	default:
		throw AssetLoadException<Mesh>(key, "Unknown format");
	}
}
